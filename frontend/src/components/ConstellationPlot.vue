...
interface PlotModel {
  range: number
  inliers: ConstellationPoint[]
  outliers: ConstellationPoint[]
  total: number
}
...
function niceCeil(v: number): number {
  ...
  for (const m of [1, 1.5, 2, 2.5, 3, 4, 5, 6, 8, 10]) {
    if (m * base >= v * 0.999999999) return parseFloat((m * base).toPrecision(12))
  }
  return 10 * base
}
...
const model = computed<PlotModel | null>(() => {
  ...
  const inliers: ConstellationPoint[] = []
  const outliers: ConstellationPoint[] = []
  for (const p of pts) {
...
const scaleNote = computed(() => {
  ...
  if (m.outliers.length) s += ` · ${m.outliers.length} 点越界，已按边缘 ▲ 标记`
  return s
})
...
function draw() {
  ...
  const toX = (i: number) => cx + (i / range) * half
  const toY = (q: number) => cy - (q / range) * half
  ...
  for (const t of ticks) {
    const x = toX(t * range)
    ctx.beginPath(); ctx.moveTo(x, cy - half); ctx.lineTo(x, cy + half)
...
.scale-note { margin-top:8px; font-size:11px; color:#7a8a9a; text-align:center }
</style>
